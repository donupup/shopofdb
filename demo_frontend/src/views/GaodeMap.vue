...
export default {
  ...
  data() {
    let self = this
    return {
      searchKey: this.$route.query.addr, //this.$store.state.address.addr,
      amapManager,
      markers: [],
      searchOption: {
        city: '全国',
          ...
          geocoder.getAddress([lng, lat], function(status, result) {
            if (status === 'complete' && result.info === 'OK') {//
              if (result && result.regeocode) {
                console.log(result.regeocode) //控制台打印地址
        ...
        AMapUI.loadUI(['misc/PoiPicker'], function (PoiPicker) {
          ...
          poiPicker.on('poiPicked', function (poiResult) {
            ...
            if (source !== 'search') {
              ...
              let address = poi.pname+poi.cityname + poi.adname + poi.name
              vm.center = [lng, lat]
